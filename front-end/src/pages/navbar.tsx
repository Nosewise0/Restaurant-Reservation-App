    ...
    return (
        <>
            ...
            <div
                ...
                className={`fixed inset-0 bg-black/60 backdrop-blur-xs transition-opacity duration-300 z-50 ${isDrawerOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
                    }`}
            />


            <div
                className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-[#8B2020] text-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col justify-between p-8 ${isDrawerOpen ? "translate-x-0" : "translate-x-full"
                    }`}
            >
                <div>
                    ...
                    <ul className="mt-8 flex flex-col gap-6 font-inika text-2xl">
                        {["Home", "Menu", "Takeout", "Reserve", "Our Story", "Contact"].map((item) => (
                            <li
                                key={item}
                                onClick={() => setIsDrawerOpen(false)}
